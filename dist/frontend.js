...
class CanvasWeatherRenderer {
  ...
  drawBack(time) {
    ...
    this.drawThermalShimmer(time, profile);
    ...
    this.drawWorldInteraction(time, profile, lightning.flash, contactBands, impactBursts, runoffSheets, accumulationBands, fogCreep);
...
function createSettingsUI(sendToBackend) {
  ...
  const motionSection = createSection("Motion", "Fine-tune motion budgets and pacing. Cinematic intentionally pushes the heaviest cloud, contact, precipitation, and glass interaction layers.");
...
{{weather_format}}`));
  ...
  qualityHint.textContent = "Performance and Lite cut scene depth, world-contact passes, and particle counts. Cinematic unlocks the richest framing, environmental interaction, pane moisture, and full weather budget for stronger devices.";
...
function createHudWidget(ctx, initialPosition, expanded, callbacks) {
  ...
  if (expanded) {
    ...
    qualitySelect.title = "Cinematic uses the highest scene depth, environmental interaction, pane moisture, and particle budget, so it is intentionally heavier.";
